...
class Frontier_Explorer(Node):
    ...
    def explore(self):
        ...
        unknown_mask = np.where(self.occupancy.probs == -1)
        unoccupied_mask = np.where((self.occupancy.probs > -1) & (self.occupancy.probs < 0.5))
        ...
        total_neighbors = window_size**2 - 1
        frontier_mask = (unknown / total_neighbors >= 0.2) & (occupied / total_neighbors == 0) & (unoccupied / total_neighbors >= 0.3)
