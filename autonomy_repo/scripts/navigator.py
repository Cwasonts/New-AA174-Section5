    ...
    def get_neighbors(self, x):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                ...
                # TO DO: Compute neighbor coordinates
        ...
        while len(self.open_set) > 0:
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh not in self.closed_set:
                    # TO DO: Compute tentative cost to reach the neighbor
                    # HINT: g(x_neigh) = g(x_current) + distance(x_current, x_neigh)
                    cost_to_neighbor = self.est_total_cost[x_current] + self.distance(x_current, x_neigh)
                    ...
                    # TO DO: Record total cost of neighbor nodes
